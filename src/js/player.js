...
class XPlayer {
    ...
    constructor(options) {
        ...
        this.setting = new Setting(this, this.tran);
        ...
        this.windowResize = () => utils.debounce(this.resize, 300)();
                ...
                case 'webtorrent':
                    if (window.WebTorrent) {
                        if (window.WebTorrent.WEBRTC_SUPPORT) {
                            ...
                            video.addEventListener('durationchange', () => this.container.classList.remove('xplayer-loading'), { once: true });
    ...
    initVideo(video, type) {
        ...
        this.on('error', () => {
            ...
            this.tran && this.notice && this.type !== 'webtorrent' && this.notice(this.tran('video-failed'), undefined, undefined, 'video-failed');
        });
    ...
    switchQuality(index) {
        ...
        this.on('canplay', () => {
            if (this.prevVideo) {
                ...
                this.video.classList.remove('xplayer-video-temp');
                if (!paused) {
                    this.video.play();
                }
                this.prevVideo = null;
    ...
    notice(text, time = 2000, opacity = 0.8, id) {
        let oldNoticeEle;
        if (id) {
            oldNoticeEle = document.getElementById(`xplayer-notice-${id}`);
            if (oldNoticeEle) {
    ...
    resize = () => {
        if (this.controller.thumbnails) {
            this.controller.thumbnails.resize(this.video.videoHeight / this.video.videoWidth, this.template.barWrap.offsetWidth);
        }
